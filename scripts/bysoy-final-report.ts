...
async function main() {
  ...
  console.log(`  → 수수료 ${r.commissionRate ?? "?"}% = ${krw(r.commissionAmount)} · 폴바이스 귀속 ${krw(r.netToPaulvice)}`);

  const isFinal = r.silverHoldQty === 0;        // 보류 물량 없으면 최종 마감
  const roundLabel = isFinal ? "최종" : "1차";
  ...
  const caption = [
    ...
    r.commissionRate != null
      ? `<b>정산</b>: 수수료 ${r.commissionRate}% = ${krw(r.commissionAmount)} · 폴바이스 귀속 ${krw(r.netToPaulvice)}`
      : `<b>정산</b>: 수수료율 미설정`,
    ``,
    `<b>모델 TOP5</b>`,
    top,
  ...
  const settleLine = r.commissionRate != null
    ? `순매출 <b>${krw(r.revenue)}</b> 기준, 약정 수수료 <b>${r.commissionRate}%</b>로 <b>${krw(r.commissionAmount)}</b> 정산해 드릴 예정이에요.`
    : `정산 금액은 별도로 안내드릴게요.`;
  const draft = [
    ...
    `안녕하세요 바이소이님 😊`,
    isFinal
      ? `이번 공동구매 최종 마감 내역 정리해서 정산 보고서로 전달드려요.`
    ...
    settleLine,
    ...(isFinal
      ? [``, `잔여 배송 물량까지 모두 출고 완료되어 이번 자료로 공구 정산을 마무리합니다.`]
      : [
          ``,
  ...
  if (SEND) {
    ...
    await sendTelegramDocument(pdf, `바이소이공구_마감보고서_${roundLabel}_${r.periodEnd}.pdf`, caption);

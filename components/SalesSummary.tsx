  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {cards.map(({ label, period, color, icon: Icon }) => {
        const d = data[period];
        return (
          <div key={period} className={`bg-gradient-to-br ${color} rounded-2xl p-6 text-white shadow-lg`}>
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium opacity-80">{label}</span>
              <div className="bg-white/20 rounded-xl p-2"><Icon size={18} /></div>
            </div>
            <div className="text-3xl font-bold mb-3 tracking-tight">{fmt(d.revenue)}</div>

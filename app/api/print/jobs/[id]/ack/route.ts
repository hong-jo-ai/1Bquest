...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  let body: { status?: string; error?: string; printer?: string; output?: string } = {};
  ...
  const next = {
    ...
    output: typeof body.output === "string" ? body.output.slice(0, 20000) : prev.output ?? null, // cmd 잡의 실행 출력
  };

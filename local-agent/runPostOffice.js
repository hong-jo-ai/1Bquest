...
(async function main() {
  ...
  // 3) 스마트스토어 발송처리 — 접수 = 실출고인데 발송처리 전까지는 고객이 버튼 하나로 취소할 수 있다.
  //    9/16 에 이 호출을 접수 큐 워커(단건)에만 넣고 여기(정기 일괄)엔 안 넣어서, 12:30 접수분은
  //    17:10 dispatch17 까지 취소 창이 열려 있었다. 2026-09-21 그 사이 1건이 취소돼 헛나갈 뻔했다.
  //    멱등(PAYED 만 대상)이라 등록이 부분 실패해도 돌린다. 실패하면 17:10 에 다시 잡힌다.
  if (!run("smartstoreDispatch.js", "스마트스토어 발송처리(취소 창 닫기)")) fails.push("스마트스토어 발송처리 — 17:10 까지 취소 가능 상태");

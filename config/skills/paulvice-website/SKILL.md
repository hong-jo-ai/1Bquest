...
### ⭐ 모바일이 항상 최우선 (사장님 2026-08-24)
자사몰 트래픽의 **91%가 모바일**이다. CSS 는 **모바일이 기본값**, 데스크톱은 `@media(min-width:768px)` 에서만 펼친다.

### 레퍼런스 판단: 브레다 O · 다니엘웰링턴 X
모바일 실측 비교 —

| | BREDA | Daniel Wellington |
|---|---|---|
| 페이지 높이 | 3,048px | 5,000px+ |
| 배치 | 350~390px **1열** | 풀폭 배너 4~5장 + 2열 그리드(162~183px) |
| 제품 배경 | **rgb(249,245,238)** | rgb(244,244,244) |

**폴바이스는 브레다 쪽.** ①고객이 30~40대·AOV 85k·"Understated Style"(DW 는 20대 로고 인지도)
②DW 식은 캠페인 배너를 계속 갈아끼워야 유지되는데 콘텐츠 생산 여력이 없다 ③에끌라 오벌 = 빈티지 드레스 워치로 DW 의 미니멀 라운드와 결이 다르다.
단 **DW 에서 가져온 "사러 가는 여정"(혜택바·각인·기프트·후기) 전환 장치는 유지**한다. → 뼈대 브레다 + 전환장치 DW.

### 1열 원칙과 웜그레이
- 모바일은 **1열 크게**. 여러 개를 보여줄 땐 2·3열로 쪼개지 말고 **가로 슬라이드**(scroll-snap).
- 제품 컷 배경 = **웜그레이 `#F9F5EE`** + 제품 아래 **부드러운 그림자**. 흰 배경보다 훨씬 감각적으로 읽힌다.
  누끼가 있으면 이미지에 구워 넣고(그림자까지), 카페24 썸네일처럼 통제 못 하면 컨테이너 배경으로 깐다.
  ⚠️ 누끼 PNG 라도 **알파가 없는 경우가 있다**(느와르 썸네일). 밝기 임계(≥246)로 배경을 떼어낼 것.
- 검증에 **1열 확인 항목**을 넣는다 — 이미지 폭 < 화면폭 90% 면 실패 처리(2열 회귀 방지).
...
## 3-1. ⚠️ 노출·판매 판단은 GA4 로 — 감으로 답하지 말 것
"노출이 적은 것 같다", "이 색은 안 팔린다" 류 질문에 판매량만 보고 답하면 **오진한다**.
`node local-agent/ga4.js items --days 90 --grep <키워드>` → 상품별 **조회·장바구니·구매·전환율**.
인증은 kv `google_refresh_token` 재사용(서비스 계정 불필요). 상세 [[paulvice-ga4-setup]].
- 조회는 많은데 안 사면 → 상품·가격 문제
- 조회 자체가 없으면 → 노출 문제 (메인 진열·광고·카테고리 확인)
...
## 6. 사고 이력 (반복 금지)
- **승인 안 난 변경이 다른 배포에 묻어 나갔다**(2026-08-25). 승인 카드용으로 로컬 섹션 파일에 미리 넣어둔
  마크업(AS WORN 5컷)이 무관한 배포에 딸려 나가 **이미지 403 = 메인에 깨진 이미지 2개**.
  → **배포 직전 `git diff` 하듯 로컬 파일의 미승인 변경을 확인**하고, 검증 checks 에 "예상 밖 요소가 없는지"도 넣을 것.
- **검증을 SFTP 세션 안에서 돌리면 연결이 끊긴다** — 캐시 대기 수 분이면 ERR_NOT_CONNECTED 로 롤백조차 실패.
  session(업로드·index교체) → 닫고 → 검증 → 실패 시 restore.
- **index.html 안의 주석에도 파일명이 들어 있다** — `pv_main_sections.html` 로 정규식 매칭하면
  import 가 아니라 **주석을 먼저 잡아** 엉뚱한 진단을 한다. `pv_main_sections_\d+\.html` 처럼 버전까지 매칭할 것.
